...
L["Enhanced click functionality with customizable textures for unit frames"] =
    L["Enhanced click functionality with customizable textures for unit frames"] or
        "Enhanced click functionality with customizable textures for unit frames"
...
Clicker.id = "clicker"
Clicker.description = L["Enhanced click functionality with customizable textures for unit frames"] or
                          "Enhanced click functionality with customizable textures for unit frames"
Clicker.version = "2.0"
...
function FrameManager:CreateClickerFrame(unitButton)
    ...
    Utils:Debug("Attempting to create texture frame - textureEnabled: " .. tostring(settings.textureEnabled) ..
                    ", selectedTexture: " .. tostring(settings.selectedTexture))
...
function LayoutManager:IsValidFrame(frame)
    return frame and type(frame.IsVisible) == "function" and frame:IsVisible() and frame.widgets and
...
function UIManager:CreateGeneralSettings(parent, anchor)
    ...
    local customSizeCheckbox = Cell.CreateCheckButton(parent, L["Use Custom Size"] or "Use Custom Size",
        function(checked)
...
function Clicker:RegisterCallbacks()
    ...
    local callbacks = {"Cell_Init", "Cell_UnitButtonCreated", "Cell_RaidFrame_Update", "Cell_PartyFrame_Update",
                       "Cell_SoloFrame_Update", "Cell_Group_Moved", "Cell_Group_Updated", "Cell_Layout_Updated"}
...
function Clicker:RegisterEvents()
    ...
    local events = {"PLAYER_TARGET_CHANGED", "GROUP_ROSTER_UPDATE", "PLAYER_ENTERING_WORLD"}
