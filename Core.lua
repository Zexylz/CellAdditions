...
function CellIntegration:CreateUtilitiesMenu(parent)
    ...
    local menuItems = {{
        ...
        text = L["Spell Request"],
        id = "spellRequest"
    }, {
        text = L["Dispel Request"],
        id = "dispelRequest"
    }, {
        text = L["Quick Assist"],
        id = "quickAssist"
...
function CellIntegration:ReplaceUtilitiesButton()
    ...
    -- Create replacement
    local L = Cell.L or {}
    local newBtn = Cell.CreateButton(Cell.frames.optionsFrame, L["Utilities"], "accent-hover", {105, 20}, nil, nil,
        "CELL_FONT_WIDGET_TITLE", "CELL_FONT_WIDGET_TITLE_DISABLE")
...
function CellIntegration:HideAllCellTabContent()
    ...
    local tabFrames = {"generalTab", "appearanceTab", "layoutsTab", "clickCastingsTab", "indicatorsTab", "debuffsTab",
                       "aboutTab"}
    ...
    if Cell.frames.optionsFrame then
        hooksecurefunc(Cell.frames.optionsFrame, "Show", function()
            if not ns.originalOptionsFrameHeight then
                ...
                Utils:Debug("Captured original frame dimensions on Show: " .. ns.originalOptionsFrameWidth .. "x" ..
